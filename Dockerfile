...
ARG XC8_X64_PRODUCT_STRING=-x64-
ENV XC8_VERSION=${XC8_VERSION}
RUN \
    wget -qO /tmp/xc"8".run "http://ww1.microchip.com/downloads/en/DeviceDoc/xc8-v${XC8_VERSION}-full-install-linux${XC8_X64_PRODUCT_STRING}installer.run" \
    && chmod a+x /tmp/xc"8".run \
    && /tmp/xc"8".run --mode unattended --unattendedmodeui none \
